@php($useOldInput = old('ticket_form') === 'create')
...
<div id="ticketCreateModal" data-should-open="{{ $useOldInput ? 'true' : 'false' }}" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-50 px-4">
    <div class="w-full max-w-lg rounded-xl bg-white shadow-2xl">
        ...
        <form action="{{ route('admin.tickets.store') }}" method="POST">
            @csrf
            <input type="hidden" name="ticket_form" value="create">
            <div class="space-y-4 p-6">
                <label class="block">
                    <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Nama Tiket</span>
                    <input type="text" name="nama_kategori" value="{{ $useOldInput ? old('nama_kategori') : null }}" placeholder="Contoh: Tiket Masuk / Camping" class="w-full rounded-lg border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </label>

                <label class="block">
                    <span class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Deskripsi</span>
                    <textarea name="deskripsi" rows="3" placeholder="Deskripsi singkat tentang tiket ini" class="w-full rounded-lg border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ $useOldInput ? old('deskripsi') : null }}</textarea>
                </label>
